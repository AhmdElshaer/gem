          ...
          {collections.map((item) => (
            ...
            <div className="flex flex-col orange-500 w-full h-full rounded">
              ...
              <div className="flex flex-col items-start gap-3 h-1/2 w-full p-8">
                <p className="font-bold flex text-orange-500 text-xl w-full gap-2">
                  <span><img className="w-8 h-7 object-cover " src={item.icon} alt="collections_icon"/></span>
                  <span className="">{item.title}</span>
                  </p>
                <p className="px-0 w-full h-14 text-xs text-ellipsis overflow-hidden ... leading-relaxed">{item.details}</p>
