...
const CollBanner = ({image, title, description}) => {
  ...
  return(
    <div className="z-30 w-full h-[90vh] flex relative flex-col justify-center items-center">
        ...
        <form onSubmit={onSubmitHandler} className="absolute z-30 bottom-0 w-full h-[15vh] flex flex-row justify-center items-center gap-4 backdrop-blur-md bg-black/30">
          <input ref={searchValue} type="text" className="rounded p-3 w-[67%] md:w-2/5 lg:w-2/5 font-bold text-xs placeholder-shown:border-0" placeholder="Search"/>
