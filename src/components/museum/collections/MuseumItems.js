...
const MuseumItems = (props) => {
  return(
    <div className='container mx-auto flex flex-col gap-8 my-6 px-3 '>
      ...
      <div id={item?.title.trim()} key={item?.id} className=' flex relative flex-col odd:md:flex-row even:md:flex-row-reverse w-full h-100vh lg:h-[45vh] truncate whitespace-normal' >
        ...
        <div key={item?.id} className={`${index % 2 === 0 ? 'md:right-0' : 'md:left-0'} md:absolute md:top-1/2 md:-translate-y-1/2 flex flex-col justify-center items-start gap-3 h-[60%] md:h-[90%] lg:h-[60%] w-full md:w-[50%] lg:w-[45%] px-5 bg-white`}>
